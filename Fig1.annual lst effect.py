...
day_annual=real.iloc[:,156:168].mean(1)*10
night_annual=real.iloc[:,168:180].mean(1)*10

#text
day_annual_str='mean='+str(round(day_annual.mean(),2))
night_annual_str='mean='+str(round(night_annual.mean(),2))
...
grid[0].set_title(r"Day",fontsize=20)

#add kde plot
ax1 = grid[0].inset_axes([0.78,0.15,0.2,0.3],transform=grid[0].transAxes)
sns.distplot(day_annual,bins=30,color="g",
             ax=ax1)
...
ax1.set_xlim(-3,3)
ax1.text(0.65,0.7,day_annual_str,fontsize=8,transform=ax1.transAxes)


#night
p = grid[1].scatter(lons, lats, 
               c=night_annual,
               s=22.0,
               vmin=-1,
               vmax=1,
...
grid[1].set_title(r"Night",fontsize=20)
ax2 = grid[1].inset_axes([0.78,0.15,0.2,0.3],transform=grid[1].transAxes)
sns.distplot(night_annual,bins=30,color='g',
             ax=ax2)
...
ax2.set_xlim(-3,3)
ax2.text(0.65,0.7,night_annual_str,fontsize=8,transform=ax2.transAxes)

plt.subplots_adjust(left=0.05, right=0.9)

#colorbar
cb = grid.cbar_axes[0].colorbar(p,ticks=np.arange(-3, 3, 0.2))
cb.ax.tick_params(axis='y', direction='in')
cb.set_label_text(r'Annual $\theta$LST(℃)',fontsize=24)

#plt.savefig('/mnt/e/US/figure/fig1.day_night_annual.jpg',dpi=300)
plt.show()
